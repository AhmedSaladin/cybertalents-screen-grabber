...
const saveLessons = async (page, courses) => {
  try {
    for (let index = 0; index < courses.length; index++) {
      ...
      const lessonsBar = loadingBar.create(
        courses[index].lessons.length + 1,
        1,
        {
...
const saveChallenges = async (page, lesson, lessonPath) => {
  try {
    ...
    fs.mkdirSync(`${lessonPath}challenges${fileSeparator()}`, {
      recursive: true,
    });
    ...
    for (let i = 0; i < challenges.length; i++) {
      const challengePath = `${lessonPath}challenges${fileSeparator()}[${challenges[
        i
      ].level
